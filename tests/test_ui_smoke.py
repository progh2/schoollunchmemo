...
def test_about_tab_links_to_project_and_author(qapp, monkeypatch):
    from app import AUTHOR_URL, ISSUES_URL, LICENSE_NAME, REPO_URL, secrets_store
    from app.settings_dialog import SettingsDialog
    ...
    try:
        ...
        html = " ".join(
            label.text() for label in tab.findChildren(type(dialog.key_status))
        )
